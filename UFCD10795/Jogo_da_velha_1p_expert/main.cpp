...
void lerCasas(char j) {
    ...
    if (j == 'X') {
        ...
        do {
            if (ganhaPC(linha, coluna)){  //CRIAR FUNÇÃO QUE VERIFICA SE O PC GANHA COM ESTA JOGADA PARA SER FEITA
                break;
            }
            else if (bloqHumano(linha, coluna)){ //CRIAR FUNÇÃO QUE VERIFICA SE O HUMANO GANHA E O BLOQUEIA
                break;
            }
            else{
                linha = rand() % 3;
